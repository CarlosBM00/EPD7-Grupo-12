...
// Vector containing two-position vectors with the players' information: name and score.
...
// Vector containing 3 position vectors with: phrase, hint and score to be added to the player who solves the panel. For each game only one element of this vector will be selected.
...
//  Variable used to store the element of the contenidosDisponibles vector that has been selected for the current game. This will be a vector of 3 positions: Phrase, hint and punctuation. 
...
// Ver funcion inicializaParametros(), donde se inicializa esta variable.
// Vector containing the possible options when spinning the roulette wheel. It consists of several numeric values, along with two string values ("Pierde turno" y "Quiebra").
...
const PIERDE_TURNO = "Pierde turno"; // El usuario pierde el turno
const QUIEBRA = "Quiebra"; // El usuario pierde el turno y su puntuacion se ve reducida a 0
// Variable usada para almacenar la última opción que ha sido seleccionada al tirar la ruleta.
// Variable used to store the last option that was selected when the roulette wheel was thrown.
